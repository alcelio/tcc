 ...
 * guests is invited. The sequence is as follows: the user selects a number
 * guests to attend the party from 0 to 1000, using the slider on the UI. When
 * the party starts, the host creates N guest agents, each of which registers
 * with the DF, and sends the host a message to say that they have arrived. When
 * all the guests have arrived, the party starts. The host selects one guest at
 * random, and tells them a rumour. The host then selects two other guests at
 * random, and introduces them to each other. The party then proceeds as
 * follows: each guest that is introduced to someone asks the host to introduce
 * them to another guest (at random). If a guest has someone introduce
 * themselves, and the guest knows the rumour, they tell the other guest. When a
 * guest hears the rumour for the first time, they notify the host. When all the
 * guests have heard the rumour, the party ends and the guests leave.
 * </p>
 * <p>
 ...
 *         <a href="mailto:Ian_Dickinson@hp.com">email</a>)
...
public class HostAgent extends Agent {
	...
	/**
	 * Invite a number of guests, as determined by the given parameter. Clears
	 * old state variables, then creates N guest agents. A list of the agents is
	 * maintained, so that the host can tell them all to leave at the end of the
	 * party.
	 *
	 * @param nGuests
	 *            The number of guest agents to invite.
	 */
	...
	/**
	 * Increment the number of guests that have heard the rumour, and update the
	 * UI. If all guests have heard the rumour, end the party.
	 */
	protected void incrementRumourCount() {
		...
		if (m_rumourCount == m_guestCount) {
			// simulate the user clicking stop when the guests have all heard
			// the rumour
			try {
	...
	/**
	 * Update the average number of introductions per rumour spread in the UI.
	 */
	protected void updateRumourAvg() {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				((HostUIFrame) m_frame).lbl_rumourAvg
						.setText(m_avgFormat.format(((double) m_introductionCount) / m_rumourCount));
			}
		});
	}
